...
class ServiceManager(Module.Module):
    ...
    def restoreWSTUN(self, wstun_process_list, servs_conf, service_uuid):
        service_name = servs_conf['services'][service_uuid]['name']
        service_pid = servs_conf['services'][service_uuid]['pid']
        LOG.info(" - " + service_name)
        ...
        if len(wstun_process_list) != 0:
            ...
            for wp in wstun_process_list:
                ...
                if service_pid == wp.pid:
                    ...
                    LOG.warning(" --> PID " + service_name + " killed!")

                    # Create the reverse tunnel again
                    public_port = \
                        servs_conf['services'][service_uuid]['public_port']
                    local_port = \
                        servs_conf['services'][service_uuid]['local_port']
                    ...
                    if wstun != None:
                        ...
                        # 3. Update services.json file
                        servs_conf['services'][service_uuid]['pid'] = \
                            service_pid
                        servs_conf['services'][service_uuid]['updated_at'] = \
                            datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%f')
    ...
    def restoreWSTUNv2(self, services_conf, service_uuid):
        service_name = services_conf['services'][service_uuid]['name']
        service_pid = services_conf['services'][service_uuid]['pid']
        LOG.info(" - " + service_name)
    ...
    def restore(self):
        ...
        if len(services_conf['services']) != 0:
            ...
            for service_uuid in services_conf['services']:
                service_pid = services_conf['services'][service_uuid]['pid']
                service_name = services_conf['services'][service_uuid]['name']
                pid_servs_list.append(service_pid)
            """
            ...
            for p in psutil.process_iter():
                if (p.name() == "node"):
                    if (p.status() == psutil.STATUS_ZOMBIE):
                        ...
                        # print(p.status); print(p.cmdline()); print(p.pid)
                        wstun_process_list.append(p)
            ...
            print("WSTUN processes: " + str(wstun_process_list))

            for service_uuid in services_conf['services']:
                # t = Thread(
                #   target=self.restoreWSTUN,
                #   args=(wstun_process_list, services_conf, service_uuid,)
                # )
                # t.start()

                Thread(
                    target=self.restoreWSTUNv2,
    ...
    def _startWstun(self, public_port, local_port):
        ...
        try:
            ...
            LOG.debug('/usr/bin/wstun ' + opt_reverse + ' ' + self.wstun_url)
